...
import { getMobileCheckoutSummary } from "@/lib/mobileCheckoutApi";
import { createMobileOrder } from "@/lib/mobileOrderApi";

import CheckoutCustomer from "@/components/mobiles/checkout/CheckoutCustomer";
import CheckoutAddress from "@/components/mobiles/checkout/CheckoutAddress";
import CheckoutPayment from "@/components/mobiles/checkout/CheckoutPayment";
import CheckoutSummary from "@/components/mobiles/checkout/CheckoutSummary";
...
function MobileCheckoutContent() {
  ...
  useEffect(() => {
    const loadCheckout = async () => {
      ...
      try {
        ...
        const response = await getMobileCheckoutSummary({
          type,
          ...
          // City is optional while initially loading checkout.
          // The backend can return the available cities.
          cityId: "",
        });
        ...
        console.error("Mobile Checkout Summary Error:", err);
  ...
  const checkoutPricing = useMemo<Pricing | null>(() => {
     ...
     * IMPORTANT:
     *
     * data.order.pricing.subtotal is already calculated
     * using the final selling price.
     *
     * Therefore we do NOT subtract discount again.
     *
     * Total = discounted subtotal + delivery charge
     */
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const response = await createMobileOrder({
        ...
        primaryMobile: data.customer.mobile,
      ...
      if (response.order?._id) {
        router.push(`/mobiles/orders/${response.order._id}`);
        return;
      }
      ...
      console.error("Mobile Place Order Error:", err);
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_400px]">
        ...
        <div className="lg:sticky lg:top-5 lg:self-start">
          ...
          <button
            ...
            className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-blue-600 font-semibold text-white shadow-sm transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {placingOrder ? (
              <>
